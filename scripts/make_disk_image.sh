...
#sudo cp test.txt /tmp/nbos
#sudo mkdir /tmp/nbos/test
#sudo cp test.txt /tmp/nbos/test
sudo umount /tmp/nbos

echo "Destroying loopback device ${DEVICE}..."

# destroy loopback device
sudo losetup -d ${DEVICE}

echo "Changing ownership of ${BUILD_DIR}/main_disk.raw to $USER:$USER"

chown $USER:$USER ${BUILD_DIR}/main_disk.raw
