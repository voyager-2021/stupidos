...
void debugc(char c) {
    fputc(c, VFS_FD_STDDBG);
}

void debugs(const char* str) {
    fputs(str, VFS_FD_STDDBG);
}

void debugf(const char* fmt, ...) {
    va_list args;
    va_start(args, fmt);
    vfprintf(VFS_FD_STDDBG, fmt, args);
    va_end(args);
}

void debug_buffer(const char* msg, const void* buffer, uint32_t count) {
    fprint_buffer(VFS_FD_STDDBG, msg, buffer, count);
}

void logc(char c) {
    fputc(c, VFS_FD_STDLOG);
}

void logs(const char* str) {
    fputs(str, VFS_FD_STDLOG);
}
...
void logf(const char* fmt, ...) {
    ...
    vfprintf(VFS_FD_STDLOG, fmt, args);

    va_end(args);
}

void log_buffer(const char* msg, const void* buffer, uint32_t count) {
    fprint_buffer(VFS_FD_STDLOG, msg, buffer, count);
}
